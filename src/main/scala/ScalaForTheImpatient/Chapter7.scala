...
object Chapter7 extends Chapter(7, "Packages and Imports", {
  exercise(1) {
  ...
  exercise(2) {
    ...
    println("This block sees _root_.ScalaForTheImpatient.com.horstmann instead of _root_.com.horstmann:")
    new Something().print()
  }

  exercise(3) {
    import ScalaForTheImpatient.random._
    setSeed(77)
    val n = 5
  ...
  exercise(6, 7) {
    ...
    def copyJHM2SHM(jhm: JHM[String, Int], shm: SHM[String, Int]): Unit = {
      for ((k, v) <- jhm) shm += k -> v
    }

    val jhm = new JHM[String, Int]()
    jhm.put("one", 1)
    jhm.put("two", 2)
    jhm.put("three", 3)
    val shm = SHM[String, Int]("zero" -> 0)
    copyJHM2SHM(jhm, shm)
    printmap(jhm, start = "Java HashMap = ")
