...
object Chapter4 extends Chapter(4, "Maps and Tuples", {
  exercise(1) {
  ...
  exercise(2) {
    val pathname = "./src/main/scala/ScalaForTheImpatient/Chapter4.scala"
  ...
  exercise(5) {
    ...
    val wordCounts: scala.collection.mutable.Map[String, Int] =
      new java.util.TreeMap[String, Int]  // Scala 2.12
      // new java.util.TreeMap[String, Int].asScala  // Scala 2.13
    while (in.hasNext()) {
      val charAggregate = in.next()
      for (word <- wordRegex.findAllMatchIn(charAggregate).map(_.toString()))
        if (word != "") wordCounts(word) = wordCounts.getOrElse(word, 0) + 1
  ...
  exercise(10) {
    ...
    def findOverlaps(word1: String, word2: String, caseSensitive: Boolean = false): Map[Int, Char] = {
      ...
      var w2 = word2
      if (!caseSensitive) {
        w1 = word1.toUpperCase(); w2 = word2.toUpperCase()
      }
