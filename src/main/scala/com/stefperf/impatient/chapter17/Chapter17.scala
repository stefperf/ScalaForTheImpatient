  ...
  override def exercises() {
    ...
    exercise(2, 3) {
      def doInOrder[T, U, V](internalFun: T => Future[U], externalFun: U => Future[V]): T => Future[V] =
        input => for (intermediate <- internalFun(input); output <- externalFun(intermediate)) yield output
    ...
    exercise(4) {
      def doTogether[T, U1, U2](f1: T => Future[U1], f2: T => Future[U2]): T => Future[(U1, U2)] = { input =>
        val fut1 = f1(input)
        val fut2 = f2(input)
        for (u1 <- fut1; u2 <- fut2) yield (u1, u2)
      }
    ...
    exercise(6) {
      ...
      def readPassword(): String = { print("password: > "); scala.io.StdIn.readLine() }

      def passwordIsCorrect(pwd: String): Boolean = pwd == "secret"

      println(s"You have found the correct password '${Await.result(repeat(readPassword, passwordIsCorrect), 2.minutes)}'.")
    }

    exercise(7) {

      def countPrimes(upToN: BigInt, certainty: Int = 10): Int = {
      ...
      val n = 103
      println(s"Among the first $n integers, ${countPrimes(n)} are probably primes.")
